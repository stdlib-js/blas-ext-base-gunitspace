...
tape( 'the function has an arity of 5', function test( t ) {
	t.strictEqual( gunitspace.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns a reference to the input array', function test( t ) {
	...
	out = gunitspace( x.length, 0.0, x, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns the input array unchanged', function test( t ) {
	...
	gunitspace( 0, 0.0, x, 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );

	gunitspace( -4, 0.0, x, 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
...
tape( 'the function supports specifying a negative stride', function test( t ) {
	...
	gunitspace( 3, 3.0, x, -2, x.length-1 );
	t.deepEqual( x, expected, 'returns expected value' );
	t.end();
});
